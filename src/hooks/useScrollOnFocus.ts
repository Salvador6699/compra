...
export function useScrollOnFocus() {
    const handleFocus = (e: FocusEvent<HTMLInputElement | HTMLTextAreaElement>) => {
        ...
        // Buscamos el contenedor padre (normalmente un div.space-y-1.5 que envuelve al Label y al Input)
        // para que sea el Label el que quede pegado arriba y no se corte visualmente.
        const target = input.closest('.space-y-1\\.5') || input.closest('.space-y-2') || input;
